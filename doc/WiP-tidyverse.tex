...
\CommentTok{#  [1] "2014" "2015" "2016" "2017" "2018" "X64"}
\end{Highlighting}
\end{Shaded}
...
We will start by removing columns \texttt{X64}, \texttt{Indicator.Name}
...
Column \texttt{X64} is created automatically due to an extra comma at
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{wip }\OperatorTok{%>%}\StringTok{ }\KeywordTok{pull}\NormalTok{(X64) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{is.na}\NormalTok{(.) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{all}\NormalTok{(.)}
\CommentTok{#  [1] TRUE}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{select}\NormalTok{(}\OperatorTok{-}\NormalTok{Indicator.Name, }\OperatorTok{-}\NormalTok{Indicator.Code, }
         \OperatorTok{-}\NormalTok{X64) }\OperatorTok{%>%}\StringTok{ }
...
\StringTok{  }\KeywordTok{select}\NormalTok{(Continent, Country, Year, pctWiP)}
\CommentTok{#  # A tibble: 10 x 4}
\CommentTok{#  # Groups:   Continent, Year [10]}
\CommentTok{#     Continent Country        Year pctWiP}
\CommentTok{#     <chr>     <chr>         <dbl>  <dbl>}
\CommentTok{#   1 Africa    Guinea-Bissau  1990   20  }
\CommentTok{#   2 Africa    Rwanda         2018   61.3}
\CommentTok{#   3 Americas  Guyana         1990   36.9}
\CommentTok{#   4 Americas  Cuba           2018   53.2}
\CommentTok{#   5 Asia      Armenia        1990   35.6}
\CommentTok{#   6 Asia      Timor-Leste    2018   33.8}
\CommentTok{#   7 Europe    Sweden         1990   38.4}
\CommentTok{#   8 Europe    Sweden         2018   46.1}
\CommentTok{#   9 Oceania   New Zealand    1990   14.4}
\CommentTok{#  10 Oceania   New Zealand    2018   38.3}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#   7 Korea, Dem. People’s Rep.  -4.8  }
...
\StringTok{  }\KeywordTok{filter}\NormalTok{(}\OperatorTok{!}\KeywordTok{is.na}\NormalTok{(Continent) }\OperatorTok{&}\StringTok{ }\NormalTok{pctDiff }\OperatorTok{<=}\StringTok{ }\OperatorTok{-}\DecValTok{5}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
...
\StringTok{  }\KeywordTok{filter}\NormalTok{(Country}\OperatorTok{==}\StringTok{"Portugal"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(Country, Year, pctWiP, RankG, TotalG) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{arrange}\NormalTok{(Year)}
\CommentTok{#  # A tibble: 23 x 5}
\CommentTok{#  # Groups:   Year [23]}
\CommentTok{#     Country   Year pctWiP RankG TotalG}
\CommentTok{#     <chr>    <dbl>  <dbl> <dbl>  <int>}
\CommentTok{#   1 Portugal  1990    7.6  66      138}
\CommentTok{#   2 Portugal  1997   13    41.5    161}
\CommentTok{#   3 Portugal  1998   13    49.5    163}
\CommentTok{#   4 Portugal  1999   18.7  24      154}
\CommentTok{#   5 Portugal  2000   17.4  34      158}
\CommentTok{#   6 Portugal  2001   18.7  33      168}
\CommentTok{#   7 Portugal  2002   19.1  41      161}
\CommentTok{#   8 Portugal  2003   19.1  46      175}
\CommentTok{#   9 Portugal  2004   19.1  54      182}
\CommentTok{#  10 Portugal  2005   21.3  45.5    185}
\CommentTok{#  11 Portugal  2006   21.3  49.5    189}
\CommentTok{#  12 Portugal  2007   28.3  28      188}
\CommentTok{#  13 Portugal  2008   28.3  28      187}
\CommentTok{#  14 Portugal  2009   27.4  33      187}
\CommentTok{#  15 Portugal  2010   27.4  34      187}
\CommentTok{#  16 Portugal  2011   28.7  31      188}
\CommentTok{#  17 Portugal  2012   28.7  35      188}
\CommentTok{#  18 Portugal  2013   28.7  39      186}
\CommentTok{#  19 Portugal  2014   31.3  35      187}
\CommentTok{#  20 Portugal  2015   34.8  29      188}
\CommentTok{#  21 Portugal  2016   34.8  27      191}
\CommentTok{#  22 Portugal  2017   34.8  28      192}
\CommentTok{#  23 Portugal  2018   34.8  29      192}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{filter}\NormalTok{(Country}\OperatorTok{==}\StringTok{"Portugal"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(Country, Year, pctWiP, RankC, TotalC) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{arrange}\NormalTok{(Year)}
\CommentTok{#  # A tibble: 23 x 5}
\CommentTok{#     Country   Year pctWiP RankC TotalC}
\CommentTok{#     <chr>    <dbl>  <dbl> <dbl>  <int>}
\CommentTok{#   1 Portugal  1990    7.6  22       28}
\CommentTok{#   2 Portugal  1997   13    16.5     39}
\CommentTok{#   3 Portugal  1998   13    18.5     37}
\CommentTok{#   4 Portugal  1999   18.7  12       38}
\CommentTok{#   5 Portugal  2000   17.4  16       38}
\CommentTok{#   6 Portugal  2001   18.7  15       41}
\CommentTok{#   7 Portugal  2002   19.1  15       39}
\CommentTok{#   8 Portugal  2003   19.1  17       41}
\CommentTok{#   9 Portugal  2004   19.1  21       42}
\CommentTok{#  10 Portugal  2005   21.3  20       42}
\CommentTok{#  11 Portugal  2006   21.3  21       44}
\CommentTok{#  12 Portugal  2007   28.3  15       44}
\CommentTok{#  13 Portugal  2008   28.3  13       44}
\CommentTok{#  14 Portugal  2009   27.4  15       44}
\CommentTok{#  15 Portugal  2010   27.4  15       44}
\CommentTok{#  16 Portugal  2011   28.7  14       44}
\CommentTok{#  17 Portugal  2012   28.7  15       43}
\CommentTok{#  18 Portugal  2013   28.7  17       43}
\CommentTok{#  19 Portugal  2014   31.3  16       43}
\CommentTok{#  20 Portugal  2015   34.8  12       43}
\CommentTok{#  21 Portugal  2016   34.8  11       43}
\CommentTok{#  22 Portugal  2017   34.8  12       43}
\CommentTok{#  23 Portugal  2018   34.8  12       43}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{select}\NormalTok{(Continent, Year, Country, pctWiP) }
\CommentTok{#  # A tibble: 10 x 4}
\CommentTok{#  # Groups:   Continent, Year [10]}
\CommentTok{#     Continent  Year Country       pctWiP}
\CommentTok{#     <chr>     <dbl> <chr>          <dbl>}
\CommentTok{#   1 Africa     1990 Guinea-Bissau   20  }
