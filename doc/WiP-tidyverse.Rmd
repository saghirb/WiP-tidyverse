---
title: Women in Parliament -- tidyverse
...
cWP %>% 
  ...
  filter(row_number()==1) %>% 
  select(Continent, Country, Year, pctWiP)
```
...
cWPrankG %>% 
  filter(Country=="Portugal") %>% 
  select(Country, Year, pctWiP, RankG, TotalG) %>% 
  arrange(Year)
```
...
cWPx %>% 
  ungroup() %>% 
  filter(Country=="Portugal") %>% 
  select(Country, Year, pctWiP, RankC, TotalC) %>% 
  arrange(Year)
```
...
of its ranking subject to random variation. 
...
cWPx %>% 
  ...
  select(Continent, Year, Country, pctWiP) 
...
[^dtref]: For more information on the `tidyverse` see https://www.tidyverse.org/.
