...
We have suppressed some of the messages but left the one about variable `X66` (we 
...
```{r checkXnnNA, collapse=TRUE, message=FALSE, warning=FALSE}
...
We will also create a numeric `Year` variable and a `Ratio` of men to women in 
parliament.
...
increased to 40% (i.e. 1.5 men for each woman) in 2020. This still falls short 
...
In October 1997, the process of “German reunification” lead to the creation of 
...
having departed now). This raises the question of how the European Union average is 
...
A total of `r nrow(filter(cWPx, Continent=="Europe" & Year==1997))` European countries 
had data in 1997, `r nrow(filter(cWPx, Continent=="Europe" & Year==1997))` in 1997 
and `r nrow(filter(cWPx, Continent=="Europe" & Year==2020))` in 2020. Within Europe, 
...
In 2020, women in parliament represented 24% (i.e. a ratio of 3.17 men to each woman), 
